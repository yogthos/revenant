...
from src.generation.base_generator import GenerationConfig, render_chat_prompt
...
class TestTemplateResolution:
    ...
    def test_adapter_config_fields_reach_generation_config(self):
        ...
        model = _parse_model_config({"chat_template": "qwen", "enable_thinking": False})
        assert (model.chat_template, model.enable_thinking) == ("qwen", False)
        with patch("src.config.get_adapter_config", return_value=model):
            config = GenerationConfig.from_config("lora_adapters/x")
        assert (config.chat_template, config.enable_thinking) == ("qwen", False)
...
class TestPyTorchGenerator:
    ...
    def test_pytorch_metadata_reads_converter_metadata(self, tmp_path):
        ...
        assert (meta.template, meta.enable_thinking, meta.lora_rank) == ("qwen", False, 8)
